...
public class CommandStringFFmpeg {
    public static String[] getCommandCreadVideoFromImageAndMusic(Context context, String uriMusic, float timeLoad, float totalTime, String pathSaveVideo) {
        String pathFolderImage = MyPath.getPathTemp(context);
        File file = new File(pathFolderImage);
        if (!file.exists()) {
            file.mkdirs();
        }
        File file1 = new File(uriMusic);
